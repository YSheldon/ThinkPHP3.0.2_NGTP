(function($){
    var defaults={direction:'top',theme:'',position:{}};
    var settings;
    var margins={border:6,top:15,right:15,bottom:15,left:15}
    ...
    function getTooltipTag(_event){
        ...
        var y =_event.clientY + document.body.scrollTop - document.body.clientTop;
        if(title_.length>0){
            if(y <= 227){
                $("#interface_list").find('img').removeClass('simpletooltip right-top pastelblue');
                $("#interface_list").find('img').addClass('simpletooltip right-bottom pastelblue');
                return('<div id="simple-tooltip-'+_event.data.index+'" class="simple-tooltip '+getTooltipPosition($(_event.target))+' '+getThemeName($(_event.target))+'">'+title_+'<span class="arrow"> </span></div>');
            }else{
                return('<div id="simple-tooltip-'+_event.data.index+'" class="simple-tooltip '+getTooltipPosition($(_event.target))+' '+getThemeName($(_event.target))+'">'+title_+'<span class="arrow"> </span></div>');
            }
        }
    ...
    function mouseOut(_event){
        var exist_=$('body').find('#simple-tooltip-'+_event.data.index);
        if(exist_.length==0){
            if($.cookie('center_url')!='?module=policy_interview&action=control_show'&& $.cookie('center_url')!='?module=policy_nat&action=nat_show')
        ...
        exist_.clearQueue().stop().fadeOut(100,function(){
            exist_.remove()});
    };
...
//            $(value).bind('mouseleave',{index:index},mouseOut);
            $(value).bind('mouseover',{index:index},mouseOver);
            $(value).bind('mouseout',{index:index},mouseOut);
        })
    };
    function setSettings(settings_){
        settings=$.extend(defaults,settings_);
    };
