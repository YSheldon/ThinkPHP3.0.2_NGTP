...
function cookie($name='', $value='', $option=null) {
        ...
        if (!empty($prefix)) {// 如果前缀为空字符串将不作处理直接返回
            foreach ($_COOKIE as $key => $val) {
                if (0 === stripos($key, $prefix)) {
                    setcookie($key, '', time() - 3600, $config['path'], $config['domain'], true, NULL);
                    unset($_COOKIE[$key]);
                }
            }
        }
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            setcookie($name, '', time() - 3600, $config['path'], $config['domain'], true, NULL);
            ...
            setcookie($name, $value, $expire, $config['path'], $config['domain'], true, NULL);
            $_COOKIE[$name] = $value;
        }
    }
}
